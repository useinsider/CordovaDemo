...
class User {
    ...
    setSMSOptin(smsOptIn) {
        if (Utils.checkParameters([{ type: 'boolean', value: smsOptIn }])) {
    ...
    logoutResettingInsiderID(identifiers, insiderIDResult) {
        try {
            ...
            if (!Utils.checkParameters([{ type: 'function', value: insiderIDResult }])) {
                Utils.asyncExec(InsiderConstants.CLASS, InsiderConstants.LOGOUT_RESETTING_INSIDER_ID, [identifiersArray, insiderIDResult]).then(id => insiderIDResult(id));
            } else {
